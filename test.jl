...
spc = SoilProfile.SoilProfileCollection("pid", s, l)
show(spc)

# empty SPC
show(SoilProfile.SoilProfileCollection())

# site and layer extraction
res = SoilProfile.extract(spc, 2:6, 2:4)
show(res)

# all layers have a site
println(SoilProfile.isValid(res))

# but not all sites have layers [4,5,6]
println(SoilProfile.sitesWithoutLayers(res))
